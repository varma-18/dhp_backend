...
router.get('', auth, async (req, res, next)  => {
  const myRes = await OrganizationController.findAll(req)
  if (myRes) {
    res.json(myRes)
  }
  else {
...
router.get('/:orgId', auth, async (req, res, next)  => {
  const myRes = await OrganizationController.findOn(req)
  if (myRes) {
    res.json(myRes)
  }
  else {
...
/* update patient */
router.put(':/orgId', auth, async (req, res, next)  => {
  const myRes = await OrganizationController.update(req)
  if (myRes) {
    res.json(myRes)
  }
  else {
...
/* Delete a patient */
router.delete(':/orgId', auth, async (req, res, next)  => {
  const myRes = await OrganizationController.destroy(req)
  if (myRes) {
    res.json(myRes)
  }
  else {
...
/* Create a patient */
router.post('', auth, [
  body('firstName', 'ERR_FIRST_NAME_EMPTY').exists({checkNull: true, checkFalse: true}),
